...
async def test_1_channel_send_receive_with_headers() -> None:
    ...
    try:
        channel = await transport.channel(
            TEST_TOPIC, num_partitions=3, retention_ms=60_000
        )
        ...
        assert msg.partition >= 0 or fail(
            f"partition should be >=0, got {msg.partition}"
        )
...
async def test_2_typed_producer_consumer_roundtrip() -> None:
    ...
    try:
        channel = await transport.channel(
            TEST_TOPIC, num_partitions=1, retention_ms=60_000
        )
...
async def test_3_typed_parser_multiple_types() -> None:
    ...
    try:
        ...
        # Produce Ping
        await channel.send(
            Ping(request_id="p1", message="ping1", timestamp="")
            .model_dump_json()
            .encode(),
            key="",
            headers={
...
async def test_6_request_reply_correlation() -> None:
    ...
    try:
        ...
        async def _send_response() -> None:
            ...
            _resp_channel = await transport.channel(
                TEST_TOPIC + "-resp", num_partitions=1, retention_ms=60_000
            )
...
async def test_8_kcat_inspection() -> None:
    ...
    try:
        channel = await transport.channel(
            TEST_TOPIC, num_partitions=2, retention_ms=60_000
        )
