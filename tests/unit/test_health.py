...
class TestHealthPublisher:
    ...
    async def test_close_emits_down(self, mock_faf: AsyncMock) -> None:
        ...
        down_calls = [
            c for c in mock_faf.send.await_args_list if c.args[0].event == "down"
        ]
    ...
    async def test_message_type_header(self, mock_faf: AsyncMock) -> None:
        ...
        assert "extra_headers" not in call_args.kwargs or call_args.kwargs.get("extra_headers") is None
