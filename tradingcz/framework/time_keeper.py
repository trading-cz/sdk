...
class TimeKeeper:
    def __init__(
        self, market_clock_provider: MarketClockProvider, sync_interval_seconds: int = 3600
    ) -> None:
    ...
    def get_warning_event(self, minutes_before_close: int) -> asyncio.Event:
        ...
        if minutes_before_close not in self._market_close_warning_events:
            ...
            logger.debug(
                "New warning event for market close in %d minutes created", minutes_before_close
            )
