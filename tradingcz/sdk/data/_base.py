...
# ------------------------------------------------------------------
# _BaseDataClient — shared transport logic
# ------------------------------------------------------------------
...
class _BaseDataClient:
    ...
    Application code never references ``_BaseDataClient`` directly.

    One ``_BaseDataClient`` is created per broker scope inside
    ...
    async def _request_historical[T](  # pylint: disable=too-many-arguments,too-many-positional-arguments
        self,
        symbols: list[str],
        asset: str,
        data_type: str,
        ...
        *,
        ...
        if resp.type != "historic":
            raise RuntimeError(
                f"Expected historic DataReady, got type={resp.type}"
            )
    ...
    async def _stream[T](  # pylint: disable=too-many-arguments,too-many-positional-arguments
        self,
        symbols: list[str],
        asset: str,
        stream_type: str,
        ...
        *,
        ...
        if isinstance(resp, DataError):
            raise RuntimeError(
                f"DataError from ingestion (stream): {resp.error}"
            )
        if resp.type != "stream":
            raise RuntimeError(
                f"Expected stream DataReady, got type={resp.type}"
            )
