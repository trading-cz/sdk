...
class ServiceApp:  # pylint: disable=too-many-instance-attributes
    ...
    def stock(self) -> StockDataClient:
        ...
        if self._transport is None:
            raise ServiceNotReadyError("Call start() before accessing stock client.")
    ...
    def stock_stream(self) -> StockStreamClient:
        ...
        if self._transport is None:
            raise ServiceNotReadyError("Call start() before accessing stock stream client.")
    ...
    def options(self) -> OptionsHistoricDataClient:
        ...
        if self._transport is None:
            raise ServiceNotReadyError("Call start() before accessing options client.")
    ...
    def corporate_actions(self) -> CorporateActionsClient:
        ...
        if self._transport is None:
            raise ServiceNotReadyError("Call start() before accessing corporate actions client.")
    ...
    def with_broker(self, broker: str) -> BrokerScope:
        ...
        if self._rr is None or self._base is None:
            raise ServiceNotReadyError("Call start() before with_broker()")
        base = BaseDataClient(
            rr=self._rr,
            settings=self._kafka,
    ...
    async def publish_event(self, message: BaseModel, *, event_id: str = "", key: str = "") -> None:
        ...
        if not self._health.running:
            raise ServiceNotReadyError("Call start() before publish_event()")
        await self._faf.send(message, event_id=event_id, key=key)
