...
Subclass for specific roles:
  ...
  - ``ProviderApp`` — provider/server role (request consumer, response producers)

Lifecycle::

    async with MyService(service_id="my-app") as svc:
...
from tradingcz.sdk.config import KafkaSettings
...
from tradingcz.sdk.observability.health import HealthPublisher
