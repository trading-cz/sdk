"""Generic async request-reply client over a transport Channel.
...
via a single background consumer — efficient when multiple strategies
share the same events channel.
...
class RequestReplyClient(Generic[Req, Resp]):
    ...
    If multiple strategies run in the same pod, share one instance —
    creating multiple instances on the same channel wastes consumer
    resources and may cause partition-assignment contention.
    ...
    Lifecycle::
        ...
        async with RequestReplyClient[Req, Resp](
            channel=events_channel,
            request_serializer=JsonCodec(Req),
            response_deserializer=my_deserializer,
            request_id_of=lambda r: r.request_id,
            response_id_of=lambda r: r.request_id,
            timeout=30.0,
        ) as client:
            response = await client.request(my_request)
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            channel: Shared channel for both sending requests and
